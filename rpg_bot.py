...
from data import *
from db import init_db, get_player, save_player, exp_needed, prof_level_for_exp, prof_next_exp
...
TOKEN = "8620344298:AAGWGHoqhQacnCjPBoivtSh8u303ZBj-pwQ"
...
def back_menu(c):
    p = get_player(c.from_user.id); regen_energy(p); save_player(p)
...
def profile(c):
    ...
    w = WEAPONS[p["weapon"]]; a = ARMORS[p["armor"]]; acc = ACCESSORIES[p["accessory"]]
    text = (f"👤 *ПРОФИЛЬ*\n{LINE}\n"
            f"⭐ Ур: {p['level']} ({p['exp']}/{exp_needed(p['level'])})\n"
            f"❤️ HP: {p['hp']}/{p['max_hp']}\n"
            f"💰 Серебро: {p['silver']:,}\n"
...
def fight_turn(c):
    ...
    if mob["hp"] <= 0:
        p["exp"] += mob["exp"]; p["silver"] += mob["silver"]; p["kills"] += 1
        p["mob_kill"] += 1
...
def make_item(c):
    ...
    if ok:
        ...
        elif key in ACCESSORIES: p["accessory"] = key
        save_player(p)
    craft_category(c)
